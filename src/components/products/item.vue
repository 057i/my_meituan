    ...
    <dd>
      ...
      <p>
        <span class="s-item-tab">
          <span v-for="(v,i) in meta.tab"
                :key="v">{{v+(i < meta.tab.length -1 ? '|' : '')}}</span>
        </span>
        <span class="s-item-addr">{{meta.address}}</span>

        <span><i class="el-icon-map-location">查看地图 </i></span>
      </p>
      ...
      <ul class="deal-items">
        <li class="items" v-for="(item,index) in meta.dealItems.slice(0,2)"
            :key="index">
          <p class="deal-title">
            {{item.title}}
          </p>
          <p>
            ...
            <span class="">已售:{{item.saleNum}}</span>
          </p>
        </li>
      </ul>
    </dd>
